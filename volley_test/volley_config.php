...
$r=0;
$array = [];
$query = mysqli_query($conn, "SELECT COUNT(1) FROM volley_questions");
...
if (in_array($q, $array)) {
    ...
    $query2 = mysqli_query($conn, 'SELECT question FROM volley_questions WHERE id ='.$q);
    $raw = mysqli_fetch_array($query2);
    $a= $r+1;
    $query3 = mysqli_query($conn, 'SELECT COUNT(1) FROM volley_answers WHERE question_num = '. $q);
    $anslist = mysqli_fetch_array($query3);
    $query3 = mysqli_query($conn, 'SELECT id FROM volley_answers WHERE question_num = '.$q);
    ...
    $r = $r - 1;
    while ($k <= $anslist[0]) {
        $query4 = mysqli_query($conn, 'SELECT answer FROM volley_answers WHERE id = '. $ida[0]);
        $res = mysqli_fetch_array($query4);
        $query5 = mysqli_query($conn, 'SELECT true_answer FROM volley_answers WHERE id = '.$ida[0]);
